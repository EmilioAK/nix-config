...
- Secrets are stored as env files under `~/.pi/secrets/`.
- For tasks that might need credentials, API keys, tokens, cloud access, deploys, integrations, or external services, first check which files exist in `~/.pi/secrets/` and decide which env file may be relevant.
